...
class SoundController:
    ...
    def set_volume(self, left=None, right=None):
        ...
        Args:
            left (int, optional): 左チャンネルの音量（0-1）
            right (int, optional): 右チャンネルの音量（0-1）
        """
        if left is not None:
            self.left_volume = max(0, min(1, left))
        
        if right is not None:
            self.right_volume = max(0, min(1, right))
        ...
        if sink_list:
            ...
            if channel_count >= 2:
                # 0.0-1.0の範囲に正規化
                volumes = [self.left_volume, self.right_volume]
    ...
    def play_noise(self, duration=None):
        """
        ガウスノイズを再生する

        Args:
            duration (float, optional): 再生時間（秒）。Noneの場合はstop_noiseが呼ばれるまで再生し続ける
        """
        # デバイスIDの検証
        ...
        if duration is not None:
            # durationが指定された場合は従来通りバッファを生成して一括再生
            ...
            # durationがNoneの場合はコールバックでノイズを生成し続ける
            print(f"ガウスノイズを再生します（無限, stop_noiseまで, デバイスID: {self.device_id if self.device_id is not None else 'デフォルト'}）")
            self.is_playing = True
            ...
            def callback(outdata, frames, time_info, status):
                if not self.is_playing:
                    ...
                    return
                # 毎回ランダムノイズを生成
                samples = np.random.normal(0, 0.1, (frames,)).astype(np.float32)
                stereo_samples = np.column_stack((samples, samples))
                outdata[:] = stereo_samples
            ...
            self.stream.start()
            # 非ブロッキングなので、ここでreturnしても他の処理が可能
...
def main():
    ...
    parser.add_argument('--left', type=int, default=0.5, help='左チャンネルの音量（0-100）')
    parser.add_argument('--right', type=int, default=0.5, help='右チャンネルの音量（0-100）')
